...
typename DLL<T>::Node* DLL<T>::push_front(const T& data) {
    // проверим уникальность добавляемого элемента
    if (find(data))
        throw my_exception::MyException("Такой элемент уже есть!");
...
typename DLL<T>::Node* DLL<T>::insert(Node* next_node, const T& data) {
    ...
    if (!next_node)
        ...
        throw my_exception::MyMemoryException("Указан несуществующий узел!");
...
typename DLL<T>::Node* DLL<T>::insert(size_t ind, const T& data) {
    ...
    for (int i = 0; i < ind; i++)
        if (next_node->next)
            ...
            // указываем, что вставить новый узел не удалось
            throw my_exception::MyMemoryException("Узла с таким индексом не существует!");
...
typename DLL<T>::Node* DLL<T>::erase(Node* node) {
    ...
    if (!head)
        // указываем, что удалить узел не удалось
        throw my_exception::MyMemoryException("Список пуст!");
